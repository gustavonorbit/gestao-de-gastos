...
import 'package:educa_plus/app/providers.dart' show conteudoRepositoryProvider;
import 'package:educa_plus/services/conteudo_export_helper.dart'
    show buildConteudoTextoFinal;
...
/// Layout-only screen for registering lesson content.
class ConteudoScreen extends ConsumerStatefulWidget {
  final int aulaId;
  final int turmaId;
  ...
  @override
  ConsumerState<ConteudoScreen> createState() => _ConteudoScreenState();
}

class _ConteudoScreenState extends ConsumerState<ConteudoScreen> {
  bool _saving = false;
  bool _loading = false;
  ...
  List<String> _currentTexts() {
    return _controllers.map((c) => c.text).toList(growable: false);
  }
  ...
  Future<bool> _save() async {
    ...
    try {
      ...
      final repo = ref.read(conteudoRepositoryProvider);
      final texts = _currentTexts();

      await repo.replaceForAula(widget.aulaId, texts);

      if (!mounted) return false;

      // Preparo para exportação futura (não exporta nada agora).
      // Mantemos o valor pronto para ser usado por outros fluxos.
      // ignore: unused_local_variable
      final _exportTextoFinal = buildConteudoTextoFinal(texts);

      setState(() {
        _houveAlteracao = false;
      });

      // Provide visual confirmation of success to the user.
      AppFeedback.success(
        context,
        ...
        final shouldLeave = await showDialog<bool>(
          ...
          builder: (ctx) {
            return AlertDialog(
              ...
              content: const Text('As alterações feitas não serão salvas.'),
              actions: [
          ...
          children: [
            Row(
              children: [
                ...
                if (showRemove)
                  IconButton(
                    key: ValueKey('conteudo_remove_$index'),
                    onPressed: onRemove,
                    icon: Icon(
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                hintText: 'Ex: Português: estudo silábico, livro pág. 90–95',
